...
export const switchAIProvider=(provider:string)=>fetch('/api/ai/provider',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider})}).then(async r=>{const d=await r.json();if(!r.ok)throw Error(d.detail||'Gagal mengganti provider AI');return d as {enabled:boolean,provider:string,model:string}})
export type LogatBackup={format:'logat-syamilah-backup',version:1,annotations:Record<string,unknown>[],bookmarks:Record<string,unknown>[]}
