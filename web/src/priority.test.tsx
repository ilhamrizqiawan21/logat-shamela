...
it('keeps the current word on save failure, then saves and advances to the next word', async () => {
  ...
  await act(async () => root.render(<App />)); await click('.book-card'); await click('.word')
  await typeMeaning('berkata')
  await act(async () => { (host.querySelector('[data-editor-input]') as HTMLInputElement).dispatchEvent(new KeyboardEvent('keydown', { key: 'Tab', bubbles: true })) })
  ...
  expect(host.querySelector('.dialog-actions')!.textContent).not.toContain('Simpan & kata berikutnya')
